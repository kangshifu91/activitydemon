...
import org.asdmp.restday.MyProcessDecision;
...
public class MyProcessService{
	...
	public String getTask(String processInstanceId){
		startUp();
		String activityName = taskService.createTaskQuery().processInstanceId(processInstanceId).list().get(0).getName();
		return activityName;
	}
	...
	public void completeTask(String node,String action) {
		...
		MyProcessDecision myProcessDecision = new MyProcessDecision();
		myProcessDecision.setAction(action);
		map.put("myProcessDecision", myProcessDecision);
